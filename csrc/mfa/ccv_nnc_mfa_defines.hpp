...
public:
  ...
  // If true, destructor will endEncoding and commit. If false, caller manages lifecycle.
  bool ownsResources = true;

  CommandBatch(MTL::CommandQueue* commandQueue);
  // Construct from external command buffer + encoder (does not own them)
  CommandBatch(MTL::CommandBuffer* cmdBuf, MTL::ComputeCommandEncoder* encoder);
  ~CommandBatch();
